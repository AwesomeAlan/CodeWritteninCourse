...
%Define Statistics
p=[0.65 0.35];  %Class Prior

%Label 0: GMM Model Stats
mu0=[3 0;0 3]';
Sigma0(:,:,1)=[2 0;0 1];
Sigma0(:,:,2)=[1 0;0 2];
alpha0=[0.5 0.5];
...
for ind=1:length(dTypes)
    %Initialize Data
    D.(dTypes{ind}).x=zeros(dimension,D.(dTypes{ind}).N); 
    ...
    grid on;
    xlabel('x1');ylabel('x2');
    title([num2str(D.(dTypes{ind}).N) ' Samples Dataset Distributions']);
end
legend 'show';
...
for ind=1:length(dTypes)-1
    ...
    D.(dTypes{ind}).DMM_Est1 = fitgmdist(D.(dTypes{ind}).x(:,D.(dTypes{ind}).labels)',1);
...
if size(x,1)==2
    plot(x(1,:),x(2,:),'b.');
    xlabel('x_1'), ylabel('x_2'), title('Data and Estimated GMM Contours'), axis equal, hold on;
...
hold on, plot(x(1,ind10),x(2,ind10),'or','DisplayName','Class 0, Incorrect'); 
hold on, plot(x(1,ind01),x(2,ind01),'+r','DisplayName','Class 1, Correct'); 
hold on, plot(x(1,ind11),x(2,ind11),'+g','DisplayName','Class 1, Incorrect'); 
hold on, axis equal, grid on;
title('ERM Classification'); 
xlabel('x1'), ylabel('x2');
legend('Class 0 Correct decisions',...
    'Class 0 Wrong decisions',...
    'Class 1 Wrong decisions',...
    'Class 1 Correct decisions');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
discriminantScoreGridValues =...
    log(evalGaussian([h(:)';v(:)'],mu1,Sigma1))-log(evalGMM([h(:)';v(:)'], alpha0,mu0,Sigma0)) - logGamma_ideal;
