...
int main(int argc, char** argv) {
  ...
  const int listen_port = argc ==1 ? 55557 : atoi(argv[1]);
  ...
  while(true) {
    ...
    int sent_size = sendto(sockfd_udp, buf, rcv_size, 0, (struct sockaddr *) &servaddr_udp, addr_l);
    if (sent_size < 0) {
      cout << "error" << endl;
      close(sockfd_udp);
      return -3;
